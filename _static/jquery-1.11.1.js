...
try {
	push.apply(
		(arr = slice.call( preferredDoc.child节点 )),
		preferredDoc.child节点
	);
	// Support: Android<4.0
	// Detect silently failing push.apply
	arr[ preferredDoc.child节点.length ].nodeType;
} catch ( e ) {
...
setDocument = Sizzle.setDocument = function( node ) {
	...
	function( a, b ) {
		...
		compare = ( a.ownerDocument || a ) === ( b.ownerDocument || b ) ?
			...
			// 其他wise we know they are disconnected
			1;
	...
	function( a, b ) {
		...
		// 其他wise we need full lists of their ancestors for comparison
		cur = a;
		...
		return i ?
			...
			// 其他wise nodes in our document sort first
			ap[i] === preferredDoc ? -1 :
			bp[i] === preferredDoc ? 1 :
			0;
	};
				...
				function( elem, context, xml ) {
					...
					if ( parent ) {
						...
						if ( forward && useCache ) {
							...
							node = nodeIndex && parent.child节点[ nodeIndex ];
...
tokenize = Sizzle.tokenize = function( selector, parseOnly ) {
	...
	// 其他wise, throw an error or return tokens
	return parseOnly ?
		soFar.length :
		soFar ?
			...
			if ( match && (match[1] || !context) ) {
				...
				// HANDLE: $(#id)
					...
					if ( elem && elem.parentNode ) {
						...
						// 其他wise, we inject the element directly into the jQuery object
						this.length = 1;
						this[0] = elem;
					}
...
jQuery.acceptData = function( elem ) {
	...
	return nodeType !== 1 && nodeType !== 9 ?
		...
		// 节点 accept data unless otherwise specified; rejection can be conditional
...
jQuery.Event.prototype = {
	...
	preventDefault: function() {
		...
		// Support: IE
		// 其他wise set the returnValue property of the original event to false
		} else {
			e.returnValue = false;
		}
	},
...
function getAll( context, tag ) {
	...
	if ( !found ) {
		for ( found = [], elems = context.child节点 || context; (elem = elems[i]) != null; i++ ) {
			...
			if ( elem || elem === 0 ) {
				...
				// Convert html into DOM nodes
					...
					if ( !support.tbody ) {
						...
						j = elem && elem.child节点.length;
						while ( j-- ) {
							if ( jQuery.nodeName( (tbody = elem.child节点[j]), "tbody" ) && !tbody.child节点.length ) {
								elem.removeChild( tbody );
							}
						}
					}

					jQuery.merge( nodes, tmp.child节点 );
...
function augmentWidthOrHeight( elem, name, extra, isBorderBox, styles ) {
	var i = extra === ( isBorderBox ? "border" : "content" ) ?
		...
		4 :
		// 其他wise initialize for horizontal or vertical properties
		name === "width" ? 1 : 0,
...
jQuery.extend({
	...
	style: function( elem, name, value, extra ) {
		...
		if ( value !== undefined ) {
			...
			// 其他wise just get the value from the style object
			return style[ name ];
		}
	},
	...
	css: function( elem, name, extra, styles ) {
		...
		// 其他wise, if a way to get the computed value exists, use that
		if ( val === undefined ) {
			val = curCSS( elem, name, styles );
		}

		//convert "normal" to computed value
		if ( val === "normal" && name in cssNormal变换 ) {
			val = cssNormal变换[ name ];
		}
...
// 动画s created synchronously will run synchronously
...
function 动画( elem, properties, options ) {
	var result,
		stopped,
		index = 0,
	...
	animate: function( prop, speed, easing, callback ) {
		var empty = jQuery.isEmptyObject( prop ),
			...
			do动画 = function() {
				...
				var anim = 动画( this, jQuery.extend( {}, prop ), optall );
	...
	toggleClass: function( value, stateVal ) {
		...
		return this.each(function() {
			...
			// Toggle whole class name
				...
				// 其他wise bring back whatever was previously saved (if anything),
...
function ajaxExtend( target, src ) {
	var deep, key,
		flatOptions = jQuery.ajax设置.flatOptions || {};
					...
					for ( conv2 in converters ) {
						...
						if ( tmp[ 1 ] === current ) {
							...
							if ( conv ) {
								...
								// 其他wise, insert the intermediate dataType
...
jQuery.extend({
	...
	// with both ajax设置 and settings fields.
	// If target is omitted, writes into ajax设置.
	ajaxSetup: function( target, settings ) {
		return settings ?

			// Building a settings object
			ajaxExtend( ajaxExtend( target, jQuery.ajax设置 ), settings ) :

			// Extending ajax设置
			ajaxExtend( jQuery.ajax设置, target );
	},
	...
	ajax: function( url, options ) {
			...
			jqXHR = {
				...
				getResponseHeader: function( key ) {
					...
					if ( state === 2 ) {
						if ( !response标题栏 ) {
							response标题栏 = {};
							while ( (match = rheaders.exec( response标题栏String )) ) {
								response标题栏[ match[1].toLowerCase() ] = match[ 2 ];
							}
						}
						match = response标题栏[ key.toLowerCase() ];
					}
					return match == null ? null : match;
				},

				// Raw string
				getAllResponse标题栏: function() {
					return state === 2 ? response标题栏String : null;
				},
				...
				setRequestHeader: function( name, value ) {
					...
					if ( !state ) {
						name = request标题栏Names[ lname ] = request标题栏Names[ lname ] || name;
						request标题栏[ name ] = value;
					}
					return this;
				},
		...
		if ( !transport ) {
			...
			try {
				state = 1;
				transport.send( request标题栏, done );
			} catch ( e ) {
	...
	unwrap: function() {
		return this.parent().each(function() {
			if ( !jQuery.nodeName( this, "body" ) ) {
				jQuery( this ).replaceWith( this.child节点 );
			}
		}).end();
	}
});
...
jQuery.param = function( a, traditional ) {
	...
	if ( traditional === undefined ) {
		traditional = jQuery.ajax设置 && jQuery.ajax设置.traditional;
	}
...
// (This is still attached to ajax设置 for backward compatibility)
jQuery.ajax设置.xhr = window.ActiveXObject !== undefined ?
	// Support: IE6+
	function() {
...
var xhrId = 0,
	xhrCallbacks = {},
	xhrSupported = jQuery.ajax设置.xhr();
...
jQuery.ajaxPrefilter( "json jsonp", function( s, original设置, jqXHR ) {
	...
	if ( jsonProp || s.dataTypes[ 0 ] === "jsonp" ) {
		...
		jqXHR.always(function() {
			...
			if ( s[ callbackName ] ) {
				...
				s.jsonpCallback = original设置.jsonpCallback;
...
jQuery.parseHTML = function( data, context, keepScripts ) {
	...
	return jQuery.merge( [], parsed.child节点 );
};
